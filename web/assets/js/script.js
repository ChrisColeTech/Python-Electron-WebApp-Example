...
function animateWindowBounds(targetX, targetY, targetWidth, targetHeight, duration = 300) {
  ...
  .then((displayBounds) => {
    ...
    window.electron.ipcRenderer.on('move-to-center', () => {
      window.electron
        .getWindowBounds()
        .then((windowBounds) => {
          const centerX = displayBounds.x + (displayBounds.width - windowBounds.width) / 2
          const centerY = displayBounds.y + (displayBounds.height - windowBounds.height) / 2
...
document.querySelectorAll('.position-clickable').forEach((button) => {
  button.addEventListener('click', async function () {
    ...
    const positions = {
      ...
      'bottom-center': {
        x: display.workArea.x + (display.workArea.width - windowBounds.width) / 2,
        y: display.workArea.y + display.workArea.height - windowBounds.height,
      },
      'top-right': {
        x: display.workArea.x + display.workArea.width - windowBounds.width,
      ...
      'left-center': {
        x: display.workArea.x,
        y: display.workArea.y + (display.workArea.height - windowBounds.height) / 2,
      },
      'right-center': {
        x: display.workArea.x + display.workArea.width - windowBounds.width,
        y: display.workArea.y + (display.workArea.height - windowBounds.height) / 2,
  ...
  if (distanceToCenter.x < threshold && distanceToCenter.y < threshold) {
    ...
    for (const [key, pos] of Object.entries(positions)) {
      ...
      const adjustedPosX = Math.max(
        display.workArea.x,
        Math.min(pos.x, display.workArea.x + display.workArea.width - buttonWidth),
      )
      const adjustedPosY = Math.max(
        display.workArea.y,
        Math.min(pos.y, display.workArea.y + display.workArea.height - buttonHeight),
      )
...
async function updateChartSize() {
  ...
  if (pos && isOutside) {
    ...
    await animateWindowBounds(pos.x, pos.y, buttonWidth + 25, buttonHeight + 25, 400)
  } else {
    if (position === 'center') {
